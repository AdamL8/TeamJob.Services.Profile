...
namespace TeamJob.Services.Profile.Events.External.Handlers
{
    public class RemoveCandidateHandler : IEventHandler<RemoveCandidate>
    {
        ...
        public RemoveCandidateHandler(IMongoRepository<UserProfile, string> InProfileRepository,
        ...
        public async Task HandleAsync(RemoveCandidate InEvent)
        {
            ...
            {
                _logger.LogError($"Profile with ID : [{InEvent.ProfileId}] is not part of the Team with ID : [{InEvent.TeamId}]");
                return;
            }

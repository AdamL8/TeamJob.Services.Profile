...
{
    public class AddMemberHandler : IEventHandler<AddMember>
    {
        ...
        public AddMemberHandler(IMongoRepository<UserProfile, Guid> InProfileRepository,
                                IBusPublisher                       InBusPublisher,
                                ILogger<AddMemberHandler>           InLogger)
        {
        ...
        public async Task HandleAsync(AddMember InEvent)
        {
            ...
            {
                _logger.LogInformation($"Profile with ID : [{InEvent.ProfileId}] is already part of the Team with ID : [{InEvent.TeamId}] as a [{teamAlreadyMemberOf.Status}]");
                return;
            }

            profile.Teams.Add(new Team(InEvent.TeamId, TeamMemberStatus.Member));
            ...
            _logger.LogInformation($"Profile with ID [{InEvent.ProfileId}] was ADDED to the Team with ID : [{InEvent.TeamId}] as a [{TeamMemberStatus.Member}]");
        }
    }
}
